...
func Test_parseFromURI_PrivateToken(t *testing.T) {
	curDevfileContext := devfileCtx.NewDevfileCtx(OutputDevfileYamlPath)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parseFromURI(tt.importReference, tt.curDevfileCtx, &resolutionContextTree{}, resolverTools{token: tt.token})
